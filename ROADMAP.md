...
### Phase 2 — Editor consolidation + `Lookup_Provider_Base` ✅ shipped

**Shipped 2026-05-23 (`1d39b82`) — see Done.** The notes below describe what was built.
...
- **2026-06-21 — /post drafts + scheduling (commits `4f81188` → `4771aa7`).** Phase 2 of the composer review — the `/post` Micropub client gained scheduled posts and a hybrid drafts library, shipped to prod in three verified slices. **Backend foundation (`4f81188`):** `Syndication_Manager` only hooked `wp_after_insert_post`, so a post becoming published from `future`/`draft` never syndicated — and a Micropub `future` post would even syndicate *early* at creation. Fixed in `syndicate()` with a publish-status guard + an idempotency guard (a status journal means syndication already fired, so the several publish triggers can't double-queue) + a new `transition_post_status` hook for `future|draft|pending → publish`. New `GET /nop-indieweb/v1/drafts` (`Authoring_Routes`) lists the user's draft+future posts. **Scheduling (`91059b9`):** a "Schedule → Post later" ledger toggle reveals date/time (reusing the RSVP native inputs); the client sends `post-status:[publish]` + a UTC `published` (converted from the author's local pick) and the POST button flips to "Schedule". The Micropub *create* path had ignored post-status/published (only *update* honoured them), so `Service_Base::handle()` now applies them before insert (mirroring the `visibility=private` block), **gated to FUTURE dates** so a Swarm check-in's past `published` is untouched — a future date → WP `future` post that syndicates exactly once when cron publishes it. **Hybrid drafts (`d61edee`, `4771aa7`):** Save stores the full `formToPost()` snapshot (incl. photo blobs) in a new IndexedDB `drafts` store (DB bumped to v2 alongside the offline `posts` queue) and mirrors the *text* to a server WP draft (`post-status:draft`) for cross-device; a Drafts button (count badge) opens a slide-up drawer listing local ∪ server (`/v1/drafts`) drafts; reopen restores full fidelity locally (`restoreSnapshot`, now also restoring a scheduled time) or fetches `?q=source` for a server-only draft (kind carried from the drawer row, since the source omits post-kind); publishing a reopened draft creates the real post then deletes both copies; drafts never syndicate (the 2A status guard). **Scope note: photos live in the local draft only — the server mirror is text.** Verified end-to-end locally (the exactly-once syndication matrix; create=future/draft/publish; save→mirror→list→reopen→publish→clear) and on prod (the live Micropub/REST contract for both scheduling and drafts, then purged). See [[project-post-drafts-scheduling]].
- **2026-06-21 — Proactive syndication-failure surface + backdate guard (`4a11db6`, `f83d62a`).** Outbound syndication failures were only visible by opening the individual post; the Networks dot meant "configured", not "delivering". Added a cheap indexed `nop_indieweb_syndication_failed` flag maintained in `Syndication_Manager::update_status()` (public consts `STATUS_META`/`FAILED_FLAG_META`), a transient-cached `failure_summary()` aggregate, a `GET /nop-indieweb/v1/syndication/health` route (manage_options), a self-resolving admin `notice-warning` ("N posts failed to syndicate → Review", `includes/admin/class-syndication-notice.php`), a `SyndicationHealth` panel atop the Networks settings tab (per-network Healthy/Failing/Off badges + a failed-posts table with per-row Retry reusing the existing retry endpoint), and a `wp nop-indieweb backfill-syndication-flag` CLI to index history — which surfaced **26 real prod failures** (all `mastodon :: HTTP 429` from a historical check-in backfill, since cleared). Root cause of that flood: the Micropub path syndicated regardless of post age, so `Syndication_Manager::syndicate()` gained a **backdate guard** — skip POSSE when `published` is older than a filterable `nop_indieweb_syndicate_max_age` (default `DAY_IN_SECONDS`); a live post lands at ~now so it's unaffected, and the /post offline-queue (which sends no `published`) replays at ~now too. See [[project-syndication-health]].
...
- **2026-05-23 — Architecture Phase 2: editor consolidation + `Lookup_Provider_Base` (`1d39b82`).** Folded the Venue panel into the Post Kind panel as a kind-specific sub-view — end state two sidebars (Post Kind + Syndication). The editor panel (`admin/post-kinds-panel.js`) is a generic renderer over `Kind_Taxonomy::get_editor_panel_config()`, now keyed by a `sub_panel` field dispatched to `VenueSubPanel` (checkin), `RsvpSubPanel` (rsvp), an exercise sub-panel, and a generic `lookup:<provider>` picker (`subPanel.slice(7)` → the provider) — no hardcoded kind branches. New `includes/lookup/Lookup_Provider_Base` mirrors `Service_Base` for interactive editor lookups (parse → search → results → on-pick: write meta + sideload cover art via `Service_Base::sideload_photos()`), with `Lookup_Provider_TMDB` the first provider, registered through the `nop_indieweb_register_lookup_providers` filter and served by the `/lookup` route (`Authoring_Routes`, `edit_posts`). The Watch kind is wired to it via `'sub_panel' => 'lookup:tmdb'`. Because the `lookup:<provider>` rail is generic, Phase 3 (Listen) reduces to a new music provider + config wiring + `u-listen-of` mf2. (Logged retroactively 2026-06-21 — shipped in May but never moved to Done.)
