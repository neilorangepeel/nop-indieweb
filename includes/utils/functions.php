...
/**
 * Returns a wp_kses allowlist for the inline SVG icons the plugin renders.
 *
 * The icons are bundled, plugin-authored markup (heart, comment, repost,
 * weather glyphs), but echoing them needs an escaper that permits SVG tags —
 * esc_html() would print the source and wp_kses_post() strips <svg>. This
 * narrow allowlist passes exactly the elements/attributes our icons use, so
 * `echo wp_kses( $icon, nop_indieweb_kses_svg() )` satisfies output escaping
 * without altering the icon.
 *
 * @return array<string, array<string, bool>>
 */
function nop_indieweb_kses_svg(): array {
	$attrs = [
		'class'           => true,
		'viewbox'         => true,
		'width'           => true,
