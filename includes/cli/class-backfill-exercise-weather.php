...
/**
 * `wp nop-indieweb backfill-exercise-weather`
 *
 * Walks every exercise post that has start coordinates but no weather data,
 * then calls Weather_Fetcher::enrich_post() using the post's recorded date
 * so historical conditions are stored.
 *
 ...
 */
class Backfill_Exercise_Weather {

	/**
	 * Backfill Pirate Weather data on existing exercise posts.
	 *
	 * ## OPTIONS
	 *
	 * [--dry-run]
	...
	public function __invoke( array $args, array $assoc_args ): void {
		...
		$meta_query = [
			'relation' => 'AND',
			[ 'key' => 'nop_indieweb_exercise_start_lat', 'compare' => 'EXISTS' ],
			[ 'key' => 'nop_indieweb_exercise_start_lng', 'compare' => 'EXISTS' ],
		];
		if ( ! $force ) {
		...
		$post_ids = get_posts( [
			...
			'fields'         => 'ids',
			'no_found_rows'  => true,
			// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query -- low-frequency CLI operation
			'tax_query'      => [
				[
			...
			// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- low-frequency CLI operation
			'meta_query'     => $meta_query,
		] );

		$total = count( $post_ids );
		WP_CLI::log( sprintf( 'Found %d exercise post(s) to process.', $total ) );
		if ( 0 === $total ) {
			return;
		}
		...
		foreach ( $post_ids as $post_id ) {
			...
			$lat = (float) get_post_meta( $post_id, 'nop_indieweb_exercise_start_lat', true );
			$lng = (float) get_post_meta( $post_id, 'nop_indieweb_exercise_start_lng', true );
